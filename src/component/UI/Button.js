...
const Button = ({redirectToLogin}) => {
  return (
    <div className="grid min-h-[200px] place-content-center p-4">
      <DrawOutlineButton redirectToLogin={redirectToLogin}>Back to Login</DrawOutlineButton>
    </div>
  );
};

const DrawOutlineButton = ({ children, redirectToLogin, ...rest }) => {
  return (
    <button
      {...rest}
